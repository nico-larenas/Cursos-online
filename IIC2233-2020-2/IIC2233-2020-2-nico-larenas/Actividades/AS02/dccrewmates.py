...
class DCCrewmates(Thread):
    ...
    def run(self):
        ...
        for tri in range(len(self.tripulantes)):
            self.tripulantes[tri].start()
            tri += 1
        self.impostor.start()
        for a in range(len(self.tripulantes)):
            self.tripulantes[a].join()
            a += 1
        self.evento_termino.set()
        self.impostor.join()

        #self.diccionario_tareas
        print("El color del impostor era: "+self.impostor)
