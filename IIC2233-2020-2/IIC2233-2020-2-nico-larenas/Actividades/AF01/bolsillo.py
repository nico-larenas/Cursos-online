...
class BolsilloCriaturas(list):
    ...
    def cantidad_criaturas_estrella(self):
        ...
        for criatura in self:
            if (criatura[2]+criatura[3]+criatura[4]+criatura[5])> 400:
                estrellas.append(criatura)
        return len(estrellas)

    def __add__(self, bolsillo_enemigo):
        dif1 = (self[0].atk+self[0].sp_atk)
        dif2 = (bolsillo_enemigo[0].atk+bolsillo_enemigo[0].sp_atk)
        debil = ""
        fuerte = ""
        for criatura in self:
            if (criatura.atk+criatura.sp_atk) < dif1:
                dif1 = (criatura.atk+criatura.sp_atk)
                debil = criatura
        for enemigo in bolsillo_enemigo:
            if (enemigo[3]+enemigo[4]) > dif2:
                dif2 = (enemigo[3]+enemigo[4])
                fuerte = enemigo
        self.remove(debil)
        bolsillo_enemigo.remove(fuerte)
        self.append(fuerte)
        bolsillo_enemigo.append(debil)
