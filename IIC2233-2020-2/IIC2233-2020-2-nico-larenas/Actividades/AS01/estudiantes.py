...
class EstudianteGryffindor(Estudiante):
    ...
    def __init__(self, nombre, saludo, *args):
        super().__init__(nombre, saludo)
        self.valor = self.valor + (random.randint(BONOS_CASA["MIN_VALOR_GRYFFINDOR"], BONOS_CASA["MAX_VALOR_GRYFFINDOR"]))
...
class EstudianteSlytherin(Estudiante):
    ...
    def __init__(self, nombre, saludo, *args):
        super().__init__(nombre, saludo)
        self.ambicion = self.ambicion + (random.randint(BONOS_CASA["MIN_AMBICION_SLYTHERIN"], BONOS_CASA["MAX_AMBICION_SLYTHERIN"]))
